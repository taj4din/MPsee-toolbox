function [Udot, error, total_iters] = fdgmres(f0, xk, TVP, TVP_f, U0, params, xinit, con)
...
%         xinit = initial iterate. Udot=0 is the default. This
...
if ~(isempty(xinit))
    Udot=xinit;
    r=-dirder(U0, Udot, xk, TVP, TVP_f, f0, con)-As*f0;
end
%
%
h=zeros(kmax);
...
%
    v(:,k+1)=dirder(U0, v(:,k), xk, TVP, TVP_f, f0, con);
    normav=norm(v(:,k+1));
%
...
if ~(isempty(xinit))
    Udot=xinit;
    r=-dirder(U0, Udot, xk, TVP, TVP_f, f0)-As*f0;
end
%
%
h=zeros(kmax);
...
%      call directional derivative function
%
    v(:,k+1)=dirder(U0, v(:,k), xk, TVP, TVP_f, f0);
    normav=norm(v(:,k+1));
%
% Modified Gram-Schmidt
%
